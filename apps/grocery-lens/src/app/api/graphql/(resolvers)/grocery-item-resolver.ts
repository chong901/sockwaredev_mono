...
import { Arg, Ctx, FieldResolver, Float, ID, Mutation, Query, Resolver, Root } from "type-graphql";

@Resolver(() => GroceryItem)
export class GroceryItemResolver {
  @Query(() => [GroceryItem])
  async getGroceryItems(@Arg("filter") filter: GroceryItemFilter, @Arg("pagination") pagination: Pagination, @Ctx() { userId }: { userId: string }) {
    return GroceryItemService.getGroceryItems(userId, filter, pagination);
  }

  @Mutation(() => GroceryItem)
  async addGroceryItem(@Arg("input") input: CreateGroceryItemInput, @Ctx() { userId }: { userId: string }) {
    return GroceryItemService.addGroceryItem(userId, input);
  }

  @Mutation(() => GroceryItem)
  async updateGroceryItem(@Arg("id", () => ID) id: string, @Arg("input") input: CreateGroceryItemInput, @Ctx() { userId }: { userId: string }) {
    return GroceryItemService.updateGroceryItem(id, userId, input);
  }

  @Mutation(() => GroceryItem)
  async deleteGroceryItem(@Arg("id", () => ID) id: string, @Ctx() { userId }: { userId: string }) {
